...
export default function() {
  ...
  prestashop.on(updateEventName, event => {
    ...
    $.post(
      event.reason.productUrl,
      { ajax: "1", action: "refresh" },
      null,
      "json"
    ).then(resp => {
      this.$nextTick(function() {
        this.modules.productPageData = $(resp.product_cover_thumbnails)
          .filter("#ajax-product-images")
          .data("ajax-products");
      ...
      if (
        ...
        quantity_wanted < minimalProductQuantity &&

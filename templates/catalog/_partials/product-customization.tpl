      ...
      <form method="post" action="{$product.url}" enctype="multipart/form-data">
        <ul class="list-unstyled">
          {foreach from=$customizations.fields item="field"}
            <li class="mb-2">
              <label class="h6 d-block"> {$field.label}</label>
              {if $field.type == 'text'}
                <textarea placeholder="{l s='Your message here' d='Shop.Forms.Help'}" class="form-control w-50" maxlength="250" {if $field.required} required {/if} name="{$field.input_name}"></textarea>
        ...
        <div class="clearfix">
          <button class="btn btn-outline-dark" type="submit" name="submitCustomizedData">

      ...
      <ul class="list-unstyled">
        {foreach from=$nodes item=node}
          <li data-depth="{$depth}">
            <a href="{$node.link}">{$node.name}</a>
            {if $node.children}
              {* <span class="arrows" data-toggle="collapse"
                data-target="#exCollapsingNavbar{$node.id}">
                <i class="fa fa-angle-down arrow-right" aria-hidden="true"></i>
                <i class="fa fa-angle-up arrow-down" aria-hidden="true"></i>
              </span>
              <div class="collapse" id="exCollapsingNavbar{$node.id}">
                {categories nodes=$node.children depth=$depth+1}
              </div> *}

              <b-btn aria-controls="exCollapsingNavbar{$node.id}" variant="primary">
                <i class="fa fa-angle-up arrow-down" aria-hidden="true"></i>
              </b-btn>
              <b-collapse id="exCollapsingNavbar{$node.id}" class="mt-2">
                {categories nodes=$node.children depth=$depth+1}
              </b-collapse>
            {/if}
          </li>

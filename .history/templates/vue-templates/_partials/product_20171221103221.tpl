...
<script type="text/x-template" id="product-miniature-template">
  <article class="product-miniature"
    ...
    <div class="position-relative thumbnail-container">
      ...
      <div class="product-description-short mb-2" itemprop="description">
      ...
      <div class="product-list-actions mb-2">

        <form :action="urls.pages.cart" method="post" class="add-to-cart-or-refresh">
          <input type="hidden" name="token" value="{$static_token}">
          <input type="hidden" name="id_product" value="{$product.id}" class="product_page_product_id">
          <input type="hidden" name="id_customization" value="{if isset($product.id_customization)}{$product.id_customization}{/if}" class="product_customization_id">
          <b-button
            variant="outline-secondary"
            v-if="product.add_to_cart_url && !configuration.is_catalog"
            class="btn btn-md btn-primary add-to-cart"
            data-button-action="add-to-cart" type="submit"
            {if !$product.add_to_cart_url || $product.availability != 'last_remaining_items' && $product.availability != 'available'}
              disabled
            {/if}>

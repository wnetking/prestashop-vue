...
var productsLimit = Vue.extend({
  template: `<div>
              ...
              <button v-if="indexes.start !== 0" class="next btn btn default" @click="next">Next</button>
              <button v-if="indexes.end !== data.length" class="prev btn btn default" @click="prev">Prev</button>
            </div>`,
  props: ['data'],
  data: data,
  methods: {
